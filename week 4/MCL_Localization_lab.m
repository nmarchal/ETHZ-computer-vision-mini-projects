...
%
    ...
    predict_particles = priorbelief_particles + uk*simpar.T + 1/2*simpar.T^2*simpar.odometry_stdev*randn(1,simpar.numberOfParticles) ;
    
    if simpar.circularHallway % the hallway is assumed to be circular
        predict_particles=mod(predict_particles,simpar.domain);
    else % the hallway is assumed to be linear
        for i = 1:simpar.numberOfParticles
            if predict_particles(i)>simpar.domain
               predict_particles(i) = simpar.domain-mod(predict_particles(i),simpar.domain);
            end
            if predict_particles(i)<0
                predict_particles(i) = - predict_particles(i);
            end
        end
    end
end
...
%
    ...
    for i = 1:simpar.numberOfParticles
        ...
        while beta > particle_weights(index)
            ...
            index = index + 1 ;
            if index > simpar.numberOfParticles
                index = 1 ;
            end
        end
